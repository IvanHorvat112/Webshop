...
export const meta = mergeMeta<typeof loader, { 'routes/tv-shows+/$tvId': typeof tvIdLoader }>(
  ({ matches, params }) => {
    ...
    const { name } = detail || {};
    return [
      { title: `YouFlix - ${name} - Crew` },
      { property: 'og:title', content: `YouFlix - ${name} - Crew` },
      { property: 'og:url', content: `https://youflix.xyz/tv-shows/${params.tvId}/crew` },
      { property: 'twitter:title', content: `YouFlix - ${name} - Crew` },
    ];
  },
);
