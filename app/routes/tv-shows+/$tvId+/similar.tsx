...
export const meta = mergeMeta<typeof loader, { 'routes/tv-shows+/$tvId': typeof tvIdLoader }>(
  ({ matches, params }) => {
    ...
    const { name } = detail || {};
    return [
      { title: `YouFlix - ${name} - Similar` },
      { property: 'og:title', content: `YouFlix - ${name} - Similar` },
      {
        property: 'og:url',
        content: `https://youflix.xyz/tv-shows/${params.tvId}/similar`,
      },
      { property: 'twitter:title', content: `YouFlix - ${name} - Similar` },
    ];
  },
);
