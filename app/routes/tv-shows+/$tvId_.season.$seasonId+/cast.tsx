...
>(({ params, matches }) => {
  ...
  const { name } = detail || {};
  return [
    { title: `YouFlix - ${name} ${seasonDetail?.name || ''} - Cast` },
    {
      property: 'og:url',
      content: `https://youflix.xyz/tv-shows/${params.tvId}/season/${params.seasonId}/cast`,
    },
    { property: 'og:title', content: `YouFlix - ${name} ${seasonDetail?.name || ''} - Cast` },
    { name: 'twitter:title', content: `YouFlix - ${name} ${seasonDetail?.name || ''} - Cast` },
  ];
});
