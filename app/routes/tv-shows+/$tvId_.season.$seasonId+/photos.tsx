...
>(({ params, matches }) => {
  ...
  const { name } = detail || {};
  return [
    { title: `YouFlix - ${name} ${seasonDetail?.name || ''} - Photos` },
    {
      property: 'og:url',
      content: `https://youflix.xyz/tv-shows/${params.tvId}/season/${params.seasonId}/photos`,
    },
    { property: 'og:title', content: `YouFlix - ${name} ${seasonDetail?.name || ''} - Photos` },
    { name: 'twitter:title', content: `YouFlix - ${name} ${seasonDetail?.name || ''} - Photos` },
  ];
});
