...
export const meta = mergeMeta<typeof loader, { 'routes/tv-shows+/$tvId': typeof tvIdLoader }>(
  ({ matches, params }) => {
    ...
    const { name } = detail || {};
    return [
      { title: `YouFlix - ${name} - Cast` },
      { property: 'og:title', content: `YouFlix - ${name} - Cast` },
      { property: 'og:url', content: `https://youflix.xyz/tv-shows/${params.tvId}/cast` },
      { property: 'twitter:title', content: `YouFlix - ${name} - Cast` },
    ];
  },
);
