...
export const meta = mergeMeta<typeof loader, { 'routes/tv-shows+/$tvId': typeof tvIdLoader }>(
  ({ matches, params }) => {
    ...
    const { name } = detail || {};
    return [
      { title: `YouFlix - ${name} - Recommendations` },
      { property: 'og:title', content: `YouFlix - ${name} - Recommendations` },
      {
        property: 'og:url',
        content: `https://youflix.xyz/tv-shows/${params.tvId}/recommendations`,
      },
      { property: 'twitter:title', content: `YouFlix - ${name} - Recommendations` },
    ];
  },
);
