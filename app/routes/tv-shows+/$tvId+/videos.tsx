...
export const meta = mergeMeta<typeof loader, { 'routes/tv-shows+/$tvId': typeof tvIdLoader }>(
  ({ matches, params }) => {
    ...
    const { name } = detail || {};
    return [
      { title: `YouFlix - ${name} - Videos` },
      { property: 'og:title', content: `YouFlix - ${name} - Videos` },
      {
        property: 'og:url',
        content: `https://youflix.xyz/tv-shows/${params.tvId}/videos`,
      },
      { property: 'twitter:title', content: `YouFlix - ${name} - Videos` },
    ];
  },
);
