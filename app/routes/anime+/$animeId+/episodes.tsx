...
export const meta = mergeMeta<{}, { 'routes/anime+/$animeId': typeof animeIdLoader }>(
  ({ params, matches }) => {
    ...
    return [
      { title: `YouFlix - ${animeTitle} - Episodes` },
      {
        name: 'description',
        content: description
          ? description?.replace(/<\/?[^>]+(>|$)/g, '')
          : `Watch ${animeTitle} in YouFlix`,
      },
      {
        property: 'og:url',
        content: `https://youflix.xyz/anime/${params.animeId}/episodes`,
      },
      { property: 'og:title', content: `YouFlix - ${animeTitle} - Episodes` },
      {
      ...
      { name: 'twitter:title', content: `YouFlix - ${animeTitle} - Episodes` },
      {
