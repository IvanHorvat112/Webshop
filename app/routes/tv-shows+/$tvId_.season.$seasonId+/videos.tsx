...
>(({ params, matches }) => {
  ...
  const { name } = detail || {};
  return [
    { title: `YouFlix - ${name} ${seasonDetail?.name || ''} - Videos` },
    {
      property: 'og:url',
      content: `https://youflix.xyz/tv-shows/${params.tvId}/season/${params.seasonId}/videos`,
    },
    { property: 'og:title', content: `YouFlix - ${name} ${seasonDetail?.name || ''} - Videos` },
    { name: 'twitter:title', content: `YouFlix - ${name} ${seasonDetail?.name || ''} - Videos` },
  ];
});
