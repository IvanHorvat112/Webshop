...
export const meta = mergeMeta<typeof loader, { 'routes/tv-shows+/$tvId': typeof tvIdLoader }>(
  ({ matches, params }) => {
    ...
    const { name } = detail || {};
    return [
      { title: `YouFlix - ${name} - Photos` },
      { property: 'og:title', content: `YouFlix - ${name} - Photos` },
      {
        property: 'og:url',
        content: `https://youflix.xyz/tv-shows/${params.tvId}/photos`,
      },
      { property: 'twitter:title', content: `YouFlix - ${name} - Photos` },
    ];
  },
);
