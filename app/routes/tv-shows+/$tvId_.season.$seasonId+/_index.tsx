...
>(({ params, matches }) => {
  ...
  const { name } = detail || {};
  return [
    { title: `YouFlix - ${name} ${seasonDetail?.name || ''}` },
    {
      property: 'og:url',
      content: `https://youflix.xyz/tv-shows/${params.tvId}/season/${params.seasonId}/`,
    },
    { property: 'og:title', content: `YouFlix - ${name} ${seasonDetail?.name || ''}` },
    { name: 'twitter:title', content: `YouFlix - ${name} ${seasonDetail?.name || ''}` },
  ];
});
