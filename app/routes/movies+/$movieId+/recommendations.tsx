...
export const meta = mergeMeta<typeof loader, { 'routes/movies+/$movieId': typeof movieIdLoader }>(
  ({ params, matches }) => {
    ...
    return [
      { title: `YouFlix - ${movieTitle} - Recommendations` },
      {
        property: 'og:url',
        content: `https://youflix.xyz/movies/${params.movieId}/recommendations`,
      },
      { property: 'og:title', content: `YouFlix - ${movieTitle} - Recommendations` },
      { name: 'twitter:title', content: `YouFlix - ${movieTitle} - Recommendations` },
    ];
  },
);
