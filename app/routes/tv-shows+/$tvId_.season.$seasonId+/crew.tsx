...
>(({ params, matches }) => {
  ...
  const { name } = detail || {};
  return [
    { title: `YouFlix - ${name} ${seasonDetail?.name || ''} - Crew` },
    {
      property: 'og:url',
      content: `https://youflix.xyz/tv-shows/${params.tvId}/season/${params.seasonId}/crew`,
    },
    { property: 'og:title', content: `YouFlix - ${name} ${seasonDetail?.name || ''} - Crew` },
    { name: 'twitter:title', content: `YouFlix - ${name} ${seasonDetail?.name || ''} - Crew` },
  ];
});
