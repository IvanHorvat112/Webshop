...
export const meta = mergeMeta<typeof loader, { 'routes/movies+/$movieId': typeof movieIdLoader }>(
  ({ params, matches }) => {
    ...
    return [
      { title: `YouFlix - ${movieTitle} - Cast` },
      {
        property: 'og:url',
        content: `https://youflix.xyz/movies/${params.movieId}/cast`,
      },
      { property: 'og:title', content: `YouFlix - ${movieTitle} - Cast` },
      { name: 'twitter:title', content: `YouFlix - ${movieTitle} - Cast` },
    ];
  },
);
